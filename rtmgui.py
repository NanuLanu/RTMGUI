...
class DBUSService(dbus.service.Object):
    def __init__(self, window):
        self.window = window
        bus_name = dbus.service.BusName('me.nanulanu.rtmgui', bus=dbus.SessionBus())
        dbus.service.Object.__init__(self, bus_name, '/me/nanulanu/rtmgui')
        args = ''
    ...
    @dbus.service.method('me.nanulanu.rtmgui', in_signature='s')
...
def call_instance():
    try:
        bus = dbus.SessionBus()
        programinstance = bus.get_object('me.nanulanu.rtmgui',  '/me/nanulanu/rtmgui')
        startup = programinstance.get_dbus_method('startup', 'me.nanulanu.rtmgui')
        args = ''
        for i, arg in enumerate(sys.argv):
            if i > 1: args += f' {arg}'
            elif i > 0: args += arg
...
class Window(Gtk.Window):
    ...
    def draw(self):
        ...
        for profile in self.profiles:
            ...
            self.widgets[profile]['name'].set_markup('<big>RTMGUI</big>\nmade by NanuLanu and LinuxHeki\n<a href="https://github.com/NanuLanu/RTMGUI">Source code</a>')
            ...
            self.widgets[profile]['pool_box'].pack_start(self.widgets[profile]['pool_entry'], False, False, 10)
            ...
            self.widgets[profile]['user_box'].pack_start(self.widgets[profile]['user_entry'], False, False, 10)
            ...
            self.widgets[profile]['pass_box'].pack_start(self.widgets[profile]['pass_entry'], False, False, 10)
            ...
            self.widgets[profile]['donate_box'].pack_start(self.widgets[profile]['donate_entry'], False, False, 10)
            ...
            self.widgets[profile]['threads_box'].pack_start(self.widgets[profile]['threads_entry'], False, False, 10)
            ...
            self.widgets[profile]['args_box'].pack_start(self.widgets[profile]['args_entry'], False, False, 10)
            ...
            self.widgets[profile]['advanched_grid'].attach(self.widgets[profile]['args_box'], 0,0,1,2)
            self.widgets[profile]['advanched_grid'].attach(self.widgets[profile]['default_args_box'], 0,2,1,2)
            self.widgets[profile]['advanched_grid'].attach(self.widgets[profile]['advanched_save_button'], 0,4,1,2)
            self.widgets[profile]['advanched_box'].pack_start(self.widgets[profile]['advanched_grid'], False, False, 15)
    ...
    def load_data(self):
        ...
        self.settings_path = f'/home/{self.user}/.config/rtmgui.json'
        self.cpuminer_path = '/opt/rtmgui/cpuminer'
        self.icon_path = '/usr/share/icons/hicolor/256x256/apps/rtmgui.png'
        self.profiles = ['profile-0', 'profile-1', 'profile-2']
        self.wallet0 = 'RWMf4o2nj7MtVRUtPnJB5FJ7fumHPeENFb'
        self.wallet1 = 'RKWdu5XxFKjpcfZqz1kKqFmaF7JRsecM5y'
        self.raw_config = '''{
    "profile-0": {
        "mine": false,
        "pool": "POOL",
    ...
    "profile-1": {
        ...
        "user": "RWMf4o2nj7MtVRUtPnJB5FJ7fumHPeENFb",
        "password": "Donate",
        "donate": "2",
        "threads": "1",
        "args": "",
    ...
    "profile-2": {
        "mine": false,
        "pool": "eu.flockpool.com:4444",
        "user": "RKWdu5XxFKjpcfZqz1kKqFmaF7JRsecM5y",
        "password": "Donate",
        "donate": "2",
        "threads": "1",
        "args": "",
...
class AppIndicator():
    def __init__(self, window):
        self.window = window
        self.indicator = AppIndicator3.Indicator.new('rtmgui', os.path.abspath(self.window.icon_path), AppIndicator3.IndicatorCategory.SYSTEM_SERVICES)
